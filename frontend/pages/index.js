...
export default function Home() {
    ...
    if (liquidityTab) {
      return (
        <div>
          <div className={styles.description}>
            You have: <br/> {utils.formatEther(cdBalance)} CryptoDev Tokens<br/>{utils.formatEther(ethBalance)} Ether <br/> {utils.formatEther(lpBalance)} CryptoDev LP Tokens
          </div>
          <div>
            {utils.parseEther(reservedCD.toString()).eq(zero) ? (
              <div>
                <input type="number" placeholder="Amount of Ether" onChange={(e) => setAddEther(e.target.value || "0")} className={styles.input} />
                <input type="number" placeholder="Amount of CryptoDev Tokens" onChange={(e) => setAddCDTokens(BigNumber.from(utils.parseEther(e.target.value || "0")))} className={styles.button} />
              ...
              <div>
                ...
                <button className={styles.button1} onClick={_addLiquidity}>Remove</button>
              </div>
            )}
            <div>
              ...
              <button className={styles.button} onClick={_removeLiquidity}>Remove</button>
            </div>
          </div>
        </div>
      );
      ...
      return (
        <div>
          ...
          <br/>
          <div className={styles.inputDiv}>
            {ethSelected ? `You will get ${utils.formatEther(tokenToBeReceivedAfterSwap)} Crypto Dev Tokens` : `You will get ${utils.formatEther(tokenToBeReceivedAfterSwap)} Eth`}
          </div>
  ...
  return (
    ...
    <footer className={styles.footer}>Made with &#10084; by pxvvxn</footer>
    </div>
  );
